...
test.describe('Describe block for hooks' , async () => {
    ...
    test('Geolocation setting in context and verification', async () => {
        context = await browser.newContext({
            ...
            geolocation: {latitude: 37.774929, longitude:-122.419416}, //using san francisco lat long
            viewport: {width: 1280, height: 720}
        })
        page = await context.newPage();
        console.log("USING CONTEXT AND PAGE CREATE WITHINT TEST AND NOT WITHIN HOOKS")
        await page.pause();
